CREATE TABLE tmp_fri_feilrettingskandidater AS (
    SELECT k.saksnummer,
           g.id AS grunnlag_id,
    ...
    WHERE k.ytelse_type = 'FRISINN'
);
CREATE TABLE bck_fri_iay_aktivitets_avtale AS (
    SELECT *
    ...
    WHERE yrkesaktivitet_id IN (
        ...
        FROM iay_yrkesaktivitet ya1 INNER JOIN iay_aktoer_arbeid aa1 ON (
            ...
            ) INNER JOIN tmp_fri_feilrettingskandidater tmp1 ON (
                aa1.inntekt_arbeid_ytelser_id = tmp1.register_id
            )
    )
);

CREATE TABLE bck_fri_iay_permisjon AS (
    SELECT *
    FROM iay_permisjon
    WHERE yrkesaktivitet_id IN (
        SELECT ya1.id
...
CREATE TABLE bck_fri_iay_yrkesaktivitet AS (
    SELECT *
...
CREATE TABLE bck_fri_iay_arbeidsforhold_refer AS (
    SELECT *
    FROM iay_arbeidsforhold_refer
    WHERE id in (
        SELECT id
        FROM iay_arbeidsforhold_refer r INNER JOIN tmp_fri_feilrettingskandidater tmp1 ON (

-- Slett tabellene fra foregående skript før de gjenopprettes nedenfor
drop table if exists tmp_omp_feilrettingskandidater;
drop table if exists bck_omp_iay_aktivitets_avtale;
drop table if exists bck_omp_iay_permisjon;
drop table if exists bck_omp_iay_yrkesaktivitet;
drop table if exists bck_omp_iay_arbeidsforhold_refer;

CREATE TABLE IF NOT EXISTS tmp_omp_feilrettingskandidater AS (
    SELECT k.saksnummer,
           g.id AS grunnlag_id,
    ...
    WHERE k.ytelse_type = 'OMP'
);
CREATE TABLE IF NOT EXISTS bck_omp_iay_aktivitets_avtale AS (
    SELECT *
    ...
    WHERE yrkesaktivitet_id IN (
        ...
        FROM iay_yrkesaktivitet ya1 INNER JOIN iay_aktoer_arbeid aa1 ON (
            ...
            ) INNER JOIN tmp_omp_feilrettingskandidater tmp1 ON (
                aa1.inntekt_arbeid_ytelser_id = tmp1.register_id
            )
    )
);

CREATE TABLE IF NOT EXISTS bck_omp_iay_permisjon AS (
    SELECT *
    FROM iay_permisjon
    WHERE yrkesaktivitet_id IN (
        SELECT ya1.id
...
CREATE TABLE IF NOT EXISTS bck_omp_iay_yrkesaktivitet AS (
    SELECT *
...
CREATE TABLE IF NOT EXISTS bck_omp_iay_arbeidsforhold_refer AS (
    SELECT *
    FROM iay_arbeidsforhold_refer
    WHERE id in (
        SELECT id
        FROM iay_arbeidsforhold_refer r INNER JOIN tmp_omp_feilrettingskandidater tmp1 ON (

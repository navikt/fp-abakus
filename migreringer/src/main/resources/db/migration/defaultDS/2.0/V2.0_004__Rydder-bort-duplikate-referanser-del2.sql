...
WHERE id IN (
    SELECT a.id
    FROM bck_psb_iay_aktivitets_avtale a
);
...
WHERE id IN (
    SELECT a.id
    FROM bck_psb_iay_yrkesaktivitet a
);
...
WHERE id IN (
    SELECT a.id
    FROM bck_psb_iay_arbeidsforhold_refer a
);
...
FROM (
   ...
   FROM prosess_task p INNER JOIN tmp_psb_feilrettingskandidater tmp1 ON (

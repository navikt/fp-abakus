...
public class TempAvledeKode {
    ...
    @SuppressWarnings("rawtypes")
    public static String getVerdiLandKoder(String classname, Object node, String key) {
        // TODO logge hvilke enum so sendes inn på gammelt format og hvor hen
        String kode;
        if (node instanceof String) {
            kode = (String) node;
            ...
            if (node instanceof JsonNode) {
                ...
                throw new IllegalArgumentException("Støtter ikke node av type: " + node.getClass() + " for enum:" + classname);
            }
            String kodeverk = "uspesifisert";
            try {
                if (node instanceof JsonNode) {

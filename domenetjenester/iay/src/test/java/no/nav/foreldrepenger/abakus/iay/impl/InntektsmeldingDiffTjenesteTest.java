...
class InntektsmeldingDiffTjenesteTest {
    ...
    void skal_ikke_finne_diff_ved_like_inntektsmeldinger() {
        ...
        Map<Inntektsmelding, ArbeidsforholdInformasjon> diff =
                InntektsmeldingDiffTjeneste.utledDifferanseIInntektsmeldinger(førsteMap, andreMap);
    ...
    void skal_lage_diff_ny_im_med_nytt_refusjonakrav() {
        ...
        Inntektsmelding inntektsmelding2 =
                lagIMf(AG1, 35_000, 35_000, UUID.randomUUID(), innsending.plusDays(1), mottatt.plusDays(1));
        Map<Inntektsmelding, ArbeidsforholdInformasjon> førsteMap = new HashMap<>();
        Map<Inntektsmelding, ArbeidsforholdInformasjon> andreMap = new HashMap<>();
        andreMap.put(inntektsmelding, new ArbeidsforholdInformasjon());
        førsteMap.put(inntektsmelding, new ArbeidsforholdInformasjon());
    ...
    private Inntektsmelding lagIMf(
            Arbeidsgiver ag,
            int inntekt,
            int refusjon,
            UUID uuid,

...
class VedtattYtelseInnhentingTjenesteTest {
    ...
    public void before() {
        vedtattYtelseInnhentingTjeneste =
                new VedtattYtelseInnhentingTjeneste(vedtakYtelseRepository, inntektArbeidYtelseRepository);
    }
    ...
    void skal_mappe_vedtatt_ytelse() throws Exception {
        ...
        VedtakYtelse vy = VedtakYtelseBuilder.oppdatere(Optional.empty())
                .medPeriode(IntervallEntitet.fraOgMedTilOgMed(
                        LocalDate.now().minusMonths(4), LocalDate.now().minusMonths(2)))
        ...
        when(inntektArbeidYtelseRepository.hentInntektArbeidYtelseGrunnlagForBehandling(any()))
        ...
        assertThat(builder.build().getAlleYtelser().stream()

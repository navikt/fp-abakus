...
class SigrunTjenesteTest {
    ...
    void skal_hente_og_mappe_om_data_fra_sigrun() {
        ...
        assertThat(inntekter.get(intervallFor(IFJOR)).get(InntektspostType.LØNN).compareTo(new BigDecimal(1000L)))
                .isZero();
        assertThat(inntekter.get(intervallFor(IFJOR.minusYears(2))).get(InntektspostType.LØNN))
                .isZero();
    }
    ...
    void skal_hente_data_for_forifjor_når_skatteoppgjoer_mangler_for_ifjor() {
        ...
        assertThat(inntekter
                        .get(intervallFor(IFJOR.minusYears(1)))
                        .get(InntektspostType.LØNN)
                        .compareTo(new BigDecimal(1000L)))
                .isZero();
        assertThat(inntekter
                        .get(intervallFor(IFJOR.minusYears(2)))
                        .get(InntektspostType.SELVSTENDIG_NÆRINGSDRIVENDE)
                        .compareTo(new BigDecimal(500L)))
                .isZero();
        assertThat(inntekter
                        .get(intervallFor(IFJOR.minusYears(2)))
                        .get(InntektspostType.LØNN)
    ...
    void skal_hente_og_mappe_om_data_fra_sigrun_opplysiningsperiode() {
        ...
        var opplysningsperiode = IntervallEntitet.fraOgMedTilOgMed(
                intervallFor(IFJOR.minusYears(2)).getFomDato(),
                intervallFor(IFJOR).getTomDato());
    ...
    void skal_hente_data_for_forifjor_når_skatteoppgjoer_mangler_for_ifjor_opplysiningsperiode() {
        ...
        assertThat(inntekter.get(intervallFor(IFJOR))).isNull();
        assertThat(inntekter
                        .get(intervallFor(IFJOR.minusYears(3)))
                        .get(InntektspostType.LØNN)
                        .compareTo(new BigDecimal(1000L)))
                .isZero();
    }
    ...
    void skal_hente_data_for_inntil_tre_år_når_skatteoppgjoer_mangler_for_ifjor_opplysiningsperiode() {
        ...
        var opplysningsperiode = IntervallEntitet.fraOgMedTilOgMed(
                intervallFor(IFJOR.minusYears(4)).getFomDato(),
                intervallFor(IFJOR).getTomDato());
    ...
    private Optional<PgiFolketrygdenResponse> lagResponsFor(Year år) {
        var inntekt = new PgiFolketrygdenResponse.Pgi(
                PgiFolketrygdenResponse.Skatteordning.FASTLAND,
                LocalDate.of(år.plusYears(1).getValue(), 6, 1),
                1000L,
                null,
                null,
                null);
    ...
    private Optional<PgiFolketrygdenResponse> lagResponsMedNæringFor(Year år) {
        var inntektF = new PgiFolketrygdenResponse.Pgi(
                PgiFolketrygdenResponse.Skatteordning.FASTLAND,
                LocalDate.of(år.plusYears(1).getValue(), 6, 1),
                500L,
                null,
                null,
                null);
        var inntektS = new PgiFolketrygdenResponse.Pgi(
                PgiFolketrygdenResponse.Skatteordning.SVALBARD,
                LocalDate.of(år.plusYears(1).getValue(), 6, 1),
                null,
                null,
                500L,
                null);
        var inntektK = new PgiFolketrygdenResponse.Pgi(
                PgiFolketrygdenResponse.Skatteordning.KILDESKATT_PAA_LOENN,
                LocalDate.of(år.plusYears(1).getValue(), 6, 1),
                500L,
                null,
                null,
                null);
        return Optional.of(new PgiFolketrygdenResponse(
                PERSONIDENT.getIdent(), år.getValue(), List.of(inntektF, inntektS, inntektK)));
    }
    ...
    private IntervallEntitet intervallFor(Year år) {
        return IntervallEntitet.fraOgMedTilOgMed(
                LocalDate.now().with(år).withDayOfYear(1),
                LocalDate.now().with(år).withDayOfYear(år.length()));
    }
}

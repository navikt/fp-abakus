...
public class MapInntektFraDtoTilDomeneTest {
    ...
    private static final AktørId FIKTIV_PRIVAT_AG = AktørId.dummy();

    @BeforeEach
    public void setup() {
        mapper = new MapInntektFraDtoTilDomene(virksomhetTjeneste, aktørTjeneste);
    }
    ...
    public void skal_mappe_inntekter_hos_en_virksomhet() {
        ...
        when(virksomhetTjeneste.hentOrganisasjonMedHensynTilJuridisk(AG1, LocalDate.of(2019,5,1))).thenReturn(lagVirksomhet(AG1));
        when(virksomhetTjeneste.sjekkOmOrganisasjonErOrgledd(any(String.class))).thenReturn(false);

        // Act
        mapper.mapFraInntektskomponent(FIKTIV_SØKER, builder, inntektsInformasjon, YtelseType.FORELDREPENGER);
    ...
    public void skal_mappe_inntekter_hos_flere_arbeidsgivere() {
        ...
        when(virksomhetTjeneste.hentOrganisasjonMedHensynTilJuridisk(AG1, LocalDate.of(2019,5,1))).thenReturn(lagVirksomhet(AG1));
        when(virksomhetTjeneste.hentOrganisasjonMedHensynTilJuridisk(AG2, LocalDate.of(2019,10,1))).thenReturn(lagVirksomhet(AG2));
        when(virksomhetTjeneste.sjekkOmOrganisasjonErOrgledd(any(String.class))).thenReturn(false);
    ...
    public void skal_mappe_inntekt_fra_juridisk_nr_til_underenhet() {
        ...
        when(virksomhetTjeneste.hentOrganisasjonMedHensynTilJuridisk(AG1, LocalDate.of(2019,5,1))).thenReturn(virksomhet);
        when(virksomhetTjeneste.hentOrganisasjonMedHensynTilJuridisk(AG2, LocalDate.of(2019,10,1))).thenReturn(virksomhet);
        when(virksomhetTjeneste.sjekkOmOrganisasjonErOrgledd(any(String.class))).thenReturn(false);
    ...
    public void skal_mappe_inntekter_hos_en_privatperson() {
        ...
        lenient().when(aktørTjeneste.hentAktørForIdent(any(), any(YtelseType.class))).thenReturn(Optional.of(FIKTIV_PRIVAT_AG));
        ...
        assertThat(alleInntekter).hasSize(1);
        Inntekt inntekt = new ArrayList<>(alleInntekter).get(0);
        assertThat(inntekt.getArbeidsgiver().getIdentifikator()).isEqualTo(FIKTIV_PRIVAT_AG.getId());
        assertThat(inntekt.getInntektsKilde()).isEqualTo(kilde);
    ...
    private void verifiserInntektsposter(List<Månedsinntekt> månedsinntekter, Collection<Inntektspost> alleInntektsposter) {
        månedsinntekter.forEach(inntekt -> {
            ...
            assertThat(matchendeInntekt.getOpprinneligUtbetalerOrgnr()).isEmpty();
    ...
    private void verifiserInntektsposterMedJuridisk(List<Månedsinntekt> vanligeInntekter, List<Månedsinntekt> inntekterFraJuridiskNr, Collection<Inntektspost> alleInntektsposter) {
        vanligeInntekter.forEach(inntekt -> {
            Inntektspost matchendeInntekt = alleInntektsposter.stream()
                ...
                .filter(ip -> ip.getOpprinneligUtbetalerOrgnr().isEmpty())
                .findFirst()
                .orElse(null);
            assertThat(matchendeInntekt).isNotNull();
            assertThat(matchendeInntekt.getBeløp().getVerdi().compareTo(inntekt.getBeløp())).isEqualTo(0);
    ...
    private boolean matcherOpprinneligUtbetaler(String juridiskNr, Inntektspost ip) {
        String oprinneligUtbetalerId = ip.getOpprinneligUtbetalerOrgnr().isEmpty() ? null : ip.getOpprinneligUtbetalerOrgnr().get().getId();

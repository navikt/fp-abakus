...
public interface TpsOversetterFeilmeldinger extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FP-112305", feilmelding = "Bruker %s er ikke Person, kan ikke hente ut adresse", logLevel = LogLevel.WARN)
    Feil ukjentBrukerTypeFinnerIkkeAdresse(String aktørId);

    @TekniskFeil(feilkode = "FP-200210", feilmelding = "Fant ikke informasjon om land i adresseinfo, bruker Norge videre", logLevel = LogLevel.WARN)
    Feil manglerLandBrukerNorge(VLException cause);

    @TekniskFeil(feilkode = "FP-222317", feilmelding = "Bruker %s: Vedtaksløsningen gjenkjenner ikke adressetype %s fra TPS", logLevel = LogLevel.WARN)

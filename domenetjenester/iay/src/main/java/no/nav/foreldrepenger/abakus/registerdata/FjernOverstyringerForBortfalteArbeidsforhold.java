...
final class FjernOverstyringerForBortfalteArbeidsforhold {
    ...
    /**
     * Identifiserer og fjerner overstyringer for arbeidsforhold som har blitt overstyrt tidligere, men som nå ikke har
     * blitt innhentet på nytt.
     *
     * <p>Dette kan være fordi arbeidsforholdet har blitt fjernet/endret i aareg til å ligge utenfor
     * opplysningsperioden.
     *
     ...
     */
    static ArbeidsforholdInformasjonBuilder fjern(
            InntektArbeidYtelseGrunnlagBuilder grunnlagBuilder,
        ...
        List<ArbeidsforholdOverstyring> overstyringerSomMåFjernes =
                finnOverstyringerForBortfalteArbeidsforhold(innhentetArbeidsforhold, informasjon);
    ...
    private static List<ArbeidsforholdOverstyring> finnOverstyringerForBortfalteArbeidsforhold(
            Set<ArbeidsforholdIdentifikator> innhentetArbeidsforhold, ArbeidsforholdInformasjon informasjon) {
        return informasjon.getOverstyringer().stream()
                ...
                .filter(ov -> innhentetArbeidsforhold.stream()
                        .noneMatch(arbeid -> arbeid.getArbeidsgiver()
                                        .getIdentifikator()
                                        .equals(ov.getArbeidsgiver().getIdentifikator())
                                && (arbeid.harArbeidsforholdRef()
                                        && arbeid.getArbeidsforholdId()
                                                .gjelderFor(informasjon.finnEkstern(
                                                        ov.getArbeidsgiver(), ov.getArbeidsforholdRef())))))

...
public class InfotrygdgrunnlagYtelseMapper {
    ...
    public static void oversettInfotrygdYtelseGrunnlagTilYtelse(
        ...
        IntervallEntitet periode =
                utledPeriodeNårTomMuligFørFom(grunnlag.getVedtaksPeriodeFom(), grunnlag.getVedtaksPeriodeTom());
        var tidligsteAnvist = grunnlag.getUtbetaltePerioder().stream()
                .map(InfotrygdYtelseAnvist::getUtbetaltFom)
        ...
        utbetaltTidslinje.toSegments().forEach(segment -> {
            ...
            if (skalMappeInfotrygdandeler(grunnlag)) {
                InfotrygdgrunnlagAnvistAndelMapper.oversettYtelseArbeidTilAnvisteAndeler(
                                grunnlag.getKategori(), grunnlag.getArbeidsforhold(), segment.getValue())
                        .forEach(anvistBuilder::leggTilYtelseAnvistAndel);
            }
            var utbetaltPeriode = IntervallEntitet.fra(
                    segment.getLocalDateInterval().getFomDato(),
                    segment.getLocalDateInterval().getTomDato());
    ...
    public static Ytelse oversettInfotrygdYtelseGrunnlagTilYtelse(InfotrygdYtelseGrunnlag grunnlag) {
        ...
        unikePerioder.forEach(intervall -> {
            var overlappendeUtbetalinger = grunnlag.getUtbetaltePerioder().stream()
                    .filter(v -> utledPeriodeNårTomMuligFørFom(v.getUtbetaltFom(), v.getUtbetaltTom())
                            .overlapper(intervall))
                    .toList();
            var anvistBuilder = ytelseBuilder.getAnvistBuilder();
            if (skalMappeInfotrygdandeler(grunnlag)) {
                InfotrygdgrunnlagAnvistAndelMapper.oversettYtelseArbeidTilAnvisteAndeler(
                                grunnlag.getKategori(), grunnlag.getArbeidsforhold(), overlappendeUtbetalinger)
                        .forEach(anvistBuilder::leggTilYtelseAnvistAndel);
            }
            ytelseBuilder.leggtilYtelseAnvist(anvistBuilder
    ...
    /**
     * For å kunne mappe utbetalinger fra infotrygd er vi avhengig av å få data som på korrekt måte kan brukes i
     * automatisk saksbehandling. Etter et omfattende arbeid på dette området vinteren 2022 er det konkludert med at
     * enkelte tilfeller må ekskluderes fordi datakvaliteten ikke er god nok til å kunne brukes til automatisk
     * saksbehandling. Disse tilfellene er: - Vedtak der det mangler dagsats - Vedtak som er basert på
     * beregningsgrunnlag uten arbeidskategori
     *
    ...
    private static boolean skalMappeInfotrygdandeler(InfotrygdYtelseGrunnlag grunnlag) {
        var harDagsatsIListeMedUtbetalinger =
                grunnlag.getUtbetaltePerioder().stream().allMatch(p -> p.getDagsats() != null);
    ...
    private static BigDecimal finnUtbetalingsgrad(List<InfotrygdYtelseAnvist> anvisninger) {
        ...
        return anvisninger.stream()
                .map(InfotrygdYtelseAnvist::getUtbetalingsgrad)
                .findFirst()
                .orElse(BigDecimal.valueOf(100));
    }

    private static YtelseGrunnlag oversettYtelseArbeid(
            InfotrygdYtelseGrunnlag grunnlag, YtelseGrunnlagBuilder grunnlagBuilder) {
    ...
    private static LocalDateSegmentCombinator<
                    List<InfotrygdYtelseAnvist>, List<InfotrygdYtelseAnvist>, List<InfotrygdYtelseAnvist>>
            slåSammenAndelslisterKombinator() {
        return (i, lhs, rhs) -> {
            if (lhs == null) {
                return rhs;
            } else if (rhs == null) {

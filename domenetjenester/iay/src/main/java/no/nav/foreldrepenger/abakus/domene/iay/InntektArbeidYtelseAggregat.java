...
public class InntektArbeidYtelseAggregat extends BaseEntitet {
    ...
    InntektArbeidYtelseAggregat(
            UUID eksternReferanse, LocalDateTime opprettetTidspunkt, InntektArbeidYtelseAggregat kopierFra) {
        Objects.requireNonNull(eksternReferanse, "eksternReferanse");
        this.setAktørInntekt(kopierFra.getAktørInntekt().stream()
                .map(ai -> {
        ...
        this.setAktørArbeid(kopierFra.getAktørArbeid().stream()
                .map(aktørArbied -> {
        ...
        this.setAktørYtelse(kopierFra.getAktørYtelse().stream()
                .map(ay -> {
    ...
    /**
     * Copy constructor - inklusiv angitt referanse og opprettet tid. Brukes for immutable copy internt i minne. Hvis
     * lagres i samme database vil det gi unik constraint exception. Men nyttig for å sende data til andre systemer.
     */
    ...
    /**
     * Identifisere en immutable instans av grunnlaget unikt og er egnet for utveksling (eks. til abakus eller andre
     * systemer)
     */
    ...
    public String toString() {
        return getClass().getSimpleName() + "<" + "aktørInntekt=" + aktørInntekt + ", aktørArbeid=" + aktørArbeid

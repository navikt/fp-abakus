...
public class InnhentingSamletTjeneste {
    ...
    private static final Set<YtelseType> LØNNSKOMP_FOR_YTELSER =
    ...
    @Inject
    public InnhentingSamletTjeneste(
    ...
    public InntektsInformasjon getInntektsInformasjon(
            AktørId aktørId, IntervallEntitet periode, InntektskildeType kilde) {
        FinnInntektRequest.FinnInntektRequestBuilder builder =
                FinnInntektRequest.builder(YearMonth.from(periode.getFomDato()), YearMonth.from(periode.getTomDato()));
    ...
    public List<Månedsinntekt> getLønnskompensasjon(AktørId aktørId, IntervallEntitet periode) {
        ...
        lønnskompensasjonRepository
                .hentLønnskompensasjonForIPeriode(aktørId, periode.getFomDato(), periode.getTomDato())
                .stream()
    ...
    public Map<ArbeidsforholdIdentifikator, List<Arbeidsforhold>> getArbeidsforhold(
            AktørId aktørId, PersonIdent ident, IntervallEntitet opplysningsPeriode) {
        return arbeidsforholdTjeneste.finnArbeidsforholdForIdentIPerioden(ident, aktørId, opplysningsPeriode);
    }

    public Map<ArbeidsforholdIdentifikator, List<Arbeidsforhold>> getArbeidsforholdFrilans(
            AktørId aktørId, PersonIdent ident, IntervallEntitet opplysningsPeriode) {
    ...
    public List<InfotrygdYtelseGrunnlag> innhentSpokelseGrunnlag(
            PersonIdent ident, @SuppressWarnings("unused") IntervallEntitet periode) {
        return innhentingInfotrygdTjeneste.getSPøkelseYtelser(ident, periode.getFomDato());
    }

    public List<MeldekortUtbetalingsgrunnlagSak> hentDagpengerAAP(
            PersonIdent ident, IntervallEntitet opplysningsPeriode) {

...
class ByggYrkesaktiviteterTjeneste {
    ...
    YrkesaktivitetBuilder byggYrkesaktivitetForSøker(
        ...
        YrkesaktivitetBuilder builder =
                aktørArbeidBuilder.getYrkesaktivitetBuilderForNøkkelAvType(opptjeningsNøkkel, arbeidsforholdType);
    ...
    private void sammenstillAktivitetsavtaler(
            List<Arbeidsforhold> arbeidsforhold, YrkesaktivitetBuilder builder, LocalDate fom) {
        finnSisteArbeidsavtale(arbeidsforhold)
                .ifPresent(arbeidsavtale ->
                        builder.leggTilAktivitetsAvtale(lagAktivitetsavtale(builder, fom, arbeidsavtale)));
    }

    private AktivitetsAvtaleBuilder lagAktivitetsavtale(
            YrkesaktivitetBuilder builder, LocalDate fom, Arbeidsavtale arbeidsavtale) {
        return builder.getAktivitetsAvtaleBuilder()
                .medProsentsats(arbeidsavtale.getStillingsprosent())
                .medSisteLønnsendringsdato(
                        arbeidsavtale.getSisteLønnsendringsdato() != null
                                ? arbeidsavtale.getSisteLønnsendringsdato()
                                : fom)
    ...
    private Optional<Arbeidsavtale> finnSisteArbeidsavtale(List<Arbeidsforhold> arbeidsforhold) {
        return arbeidsforhold.stream()
                ...
                .max(Comparator.comparing(
                        a -> a.getArbeidsavtaleTom() != null ? a.getArbeidsavtaleTom() : TIDENES_ENDE));
    }
    ...
    private void byggPermisjoner(YrkesaktivitetBuilder builder, Arbeidsforhold arbeidsforhold1) {
        final LocalDate arbeidTom =
                arbeidsforhold1.getArbeidTom() != null ? arbeidsforhold1.getArbeidTom() : TIDENES_ENDE;
        builder.tilbakestillPermisjon();
        arbeidsforhold1.getPermisjoner().stream()
    ...
    private void byggYrkesaktivitetForVirksomhet(List<Arbeidsforhold> arbeidsforhold, YrkesaktivitetBuilder builder) {
        for (Arbeidsforhold arbeid : arbeidsforhold) {
            arbeid.getArbeidsavtaler().stream()
    ...
    private Permisjon opprettPermisjoner(
            no.nav.foreldrepenger.abakus.registerdata.arbeidsforhold.Permisjon permisjon,
        ...
        return permisjonBuilder
                ...
                .medPermisjonsbeskrivelseType(
                        PermisjonsbeskrivelseType.finnForKodeverkEiersKode(permisjon.getPermisjonsÅrsak()))
                .build();
    }

    private void opprettMinimalYrkesaktivitet(
            ArbeidsforholdIdentifikator arbeidsforhold,
            Arbeidsgiver arbeidsgiver,
            InternArbeidsforholdRef internReferanse,
            YrkesaktivitetBuilder yrkesaktivitetBuilder) {
    ...
    private AktivitetsAvtaleBuilder opprettAktivitetsAvtaler(
            Arbeidsavtale arbeidsavtale, YrkesaktivitetBuilder yrkesaktivitetBuilder) {
        ...
        if (arbeidsavtale.getArbeidsavtaleTom() == null) {
            ...
            periode = IntervallEntitet.fraOgMedTilOgMed(
                    arbeidsavtale.getArbeidsavtaleFom(), arbeidsavtale.getArbeidsavtaleTom());
        }
        AktivitetsAvtaleBuilder aktivitetsAvtaleBuilder =
                yrkesaktivitetBuilder.getAktivitetsAvtaleBuilder(periode, arbeidsavtale.getErAnsettelsesPerioden());
        aktivitetsAvtaleBuilder
                .medProsentsats(arbeidsavtale.getStillingsprosent())
                .medSisteLønnsendringsdato(
                        arbeidsavtale.getSisteLønnsendringsdato() != null || arbeidsavtale.getErAnsettelsesPerioden()
                                ? arbeidsavtale.getSisteLønnsendringsdato()
                                : periode.getFomDato())
                .medPeriode(periode);

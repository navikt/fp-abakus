...
public class VirksomhetTjeneste {
    ...
    private LRUCache<String, JuridiskEnhetVirksomheter> cacheJuridiskEREG =
            new LRUCache<>(100, CACHE_ELEMENT_LIVE_TIME_MS);
    ...
    public boolean sjekkOmOrganisasjonErOrgledd(String orgNummer) {
        return OrganisasjonstypeEReg.ORGLEDD.equals(
                hentResponseOrganisasjon(orgNummer).type());
    }

    /**
     * Henter informasjon fra Enhetsregisteret hvis applikasjonen ikke kjenner til orgnr eller har data som er eldre enn
     * 24 timer. Prøver og utlede faktisk virksomhet hvis oppgitt orgNummer refererer til et juridiskOrgnummer (gjelder
     * 1 til 1 forhold) Vil lagre ned virksomheten med juridiskOrgNummer hvis det ikke er mulig og utlede faktisk
     * orgNummer (gjelder 1 til mange forhold)
     *
     * @param orgNummer orgnummeret
     * @return relevant informasjon om virksomheten.
     */
    public Virksomhet hentOrganisasjonMedHensynTilJuridisk(String orgNummer, LocalDate hentedato) {
        ...
        if (OrganisasjonType.JURIDISK_ENHET.equals(virksomhet.getOrganisasjonstype())) {
            Optional<Virksomhet> unikVirksomhetForJuridiskEnhet =
                    hentUnikVirksomhetForJuridiskEnhet(orgNummer, hentedato);
            LOG.info(
                    "ABAKUS EREG fant {} unik virksomhet for juridisk {}",
                    unikVirksomhetForJuridiskEnhet.isPresent() ? "en" : "ikke",
    ...
    private OrganisasjonEReg hentResponseOrganisasjon(String orgnr) {
        var response =
                Optional.ofNullable(cacheEREG.get(orgnr)).orElseGet(() -> eregRestKlient.hentOrganisasjon(orgnr));
        cacheEREG.put(orgnr, response);
        return response;
    }

    private JuridiskEnhetVirksomheter hentResponseJuridisk(String orgnr) {
        var response = Optional.ofNullable(cacheJuridiskEREG.get(orgnr))
                .orElseGet(() -> eregRestKlient.hentOrganisasjonHistorikk(orgnr));

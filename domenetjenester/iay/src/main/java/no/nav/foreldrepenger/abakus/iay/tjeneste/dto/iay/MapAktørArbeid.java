...
public class MapAktørArbeid {
    ...
    public List<ArbeidDto> map(Collection<AktørArbeid> aktørArbeid) {
    ...
    private ArbeidsforholdRefDto mapArbeidsforholdsId(Yrkesaktivitet yrkesaktivitet) {
        ...
        if (internId != null) {
            String eksternReferanse = tjeneste
                .finnReferanseFor(koblingReferanse, yrkesaktivitet.getArbeidsgiver(), yrkesaktivitet.getArbeidsforholdRef().orElse(null), true).getReferanse();
            return new ArbeidsforholdRefDto(internId, eksternReferanse);
        }
        return new ArbeidsforholdRefDto(internId, null);
    }

    private static Aktør mapAktør(Arbeidsgiver arbeidsgiver) {

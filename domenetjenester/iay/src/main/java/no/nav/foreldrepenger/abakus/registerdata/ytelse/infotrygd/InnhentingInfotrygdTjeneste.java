...
public class InnhentingInfotrygdTjeneste {
    ...
    private static final Map<no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.TemaKode, YtelseType>
            STØNADSKAT1_TIL_YTELSETYPE = Map.ofEntries(
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.TemaKode.UKJENT,
                            YtelseType.UDEFINERT),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.TemaKode.FA,
                            YtelseType.FORELDREPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.TemaKode.SP,
                            YtelseType.SYKEPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.TemaKode.BS,
                            YtelseType.OMSORGSPENGER));

    private static final Map<
                    no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode, YtelseType>
            STØNADSKAT2_TIL_YTELSETYPE = Map.ofEntries(
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.UKJENT,
                            YtelseType.UDEFINERT),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.AP,
                            YtelseType.FORELDREPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.FP,
                            YtelseType.FORELDREPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.FU,
                            YtelseType.FORELDREPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.FØ,
                            YtelseType.FORELDREPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.SV,
                            YtelseType.SVANGERSKAPSPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.SP,
                            YtelseType.SYKEPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.OM,
                            YtelseType.OMSORGSPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.OP,
                            YtelseType.OPPLÆRINGSPENGER),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.PP,
                            YtelseType.PLEIEPENGER_NÆRSTÅENDE),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.PB,
                            YtelseType.PLEIEPENGER_SYKT_BARN),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.PI,
                            YtelseType.PLEIEPENGER_SYKT_BARN),
                    Map.entry(
                            no.nav.vedtak.felles.integrasjon.infotrygd.grunnlag.v1.respons.BehandlingstemaKode.PN,
                            YtelseType.PLEIEPENGER_SYKT_BARN));

    private static final List<Duration> SPOKELSE_TIMEOUTS = List.of(
    ...
    public List<InfotrygdYtelseGrunnlag> getInfotrygdYtelser(PersonIdent ident, IntervallEntitet periode) {
        ...
        List<Grunnlag> rest =
                infotrygdGrunnlag.hentAggregertGrunnlag(ident.getIdent(), innhentFom, periode.getTomDato());
    ...
    public List<InfotrygdYtelseGrunnlag> getInfotrygdYtelserFailSoft(PersonIdent ident, IntervallEntitet periode) {
        LocalDate innhentFom = periode.getFomDato();
        List<Grunnlag> rest =
                infotrygdGrunnlag.hentAggregertGrunnlagFailSoft(ident.getIdent(), innhentFom, periode.getTomDato());

        return mapTilInfotrygdYtelseGrunnlag(rest, innhentFom);
    }

    public static List<InfotrygdYtelseGrunnlag> mapTilInfotrygdYtelseGrunnlag(
            List<Grunnlag> rest, LocalDate innhentFom) {
        var mappedGrunnlag = rest.stream()
                .filter(g -> !YtelseType.UDEFINERT.equals(
                        TemaReverse.reverseMap(g.tema().kode().name(), LOG)))
    ...
    private static InfotrygdYtelseGrunnlag restTilInfotrygdYtelseGrunnlag(Grunnlag grunnlag, LocalDate innhentFom) {
        if (grunnlag.iverksatt() == null
                || grunnlag.identdato() == null
                || !grunnlag.iverksatt().equals(grunnlag.identdato())) {
            LOG.info(
                    "Infotrygd ny mapper avvik iverksatt {} vs identdato {}",
                    grunnlag.iverksatt(),
                    grunnlag.identdato());
        }
        ...
        if (grunnlag.opphørFom() != null) {
            LOG.info(
                    "Infotrygdgrunnlag: OpphørFom: {}, Iverksatt: {}, Registrert: {}, Periode: {}",
                    grunnlag.opphørFom(),
                    grunnlag.iverksatt(),
                    grunnlag.registrert(),
        ...
        Integer dekningsgrad =
                grunnlag.dekningsgrad() != null ? grunnlag.dekningsgrad().prosent() : null;
        Arbeidskategori arbeidskategori = grunnlag.kategori() == null
                ? Arbeidskategori.UGYLDIG
        ...
        grunnlag.vedtak().stream()
                .map(v -> new InfotrygdYtelseAnvist(
                        v.periode().fom(),
                        v.periode().tom(),
                        new BigDecimal(v.utbetalingsgrad()),
                        v.arbeidsgiverOrgnr(),
                        v.erRefusjon(),
                        v.dagsats() != null ? BigDecimal.valueOf(v.dagsats()) : null))
    ...
    private static InfotrygdYtelseArbeid arbeidsforholdTilInfotrygdYtelseArbeid(Arbeidsforhold arbeidsforhold) {
        InntektPeriodeType inntektPeriode = arbeidsforhold.inntektsperiode() == null
                ? InntektPeriodeType.UDEFINERT
                : InntektPeriodeReverse.reverseMap(
        ...
        return new InfotrygdYtelseArbeid(
                Optional.ofNullable(arbeidsforhold.orgnr())
                        .map(Orgnummer::orgnr)
                        .orElse(null),
                inntekt,
                inntektPeriode,
    ...
    private static YtelseType bestemYtelseType(Grunnlag grunnlag) {
        YtelseType kategori2 = grunnlag.behandlingstema() == null
                ? YtelseType.UDEFINERT
                : STØNADSKAT2_TIL_YTELSETYPE.getOrDefault(
                        grunnlag.behandlingstema().kode(), YtelseType.UDEFINERT);
        if (!YtelseType.UDEFINERT.equals(kategori2)) {
            return kategori2;
        }
        LOG.info("Infotrygd ukjent stønadskategori 2");
        YtelseType kategori1 = grunnlag.tema() == null
                ? YtelseType.UDEFINERT
    ...
    public List<InfotrygdYtelseGrunnlag> getSPøkelseYtelser(PersonIdent ident, LocalDate fom) {
        ...
        while (it.hasNext()) {
            try {
                ...
                LOG.info(
                        "Spøkelse antall {} timeout {} svartid {}",
                        vedtak.size(),
                        timeout,
    ...
    private InfotrygdYtelseGrunnlag spokelseTilInfotrygdYtelseGrunnlag(SykepengeVedtak grunnlag) {
        LocalDate min = grunnlag.utbetalingerNonNull().stream()
                .map(SykepengeVedtak.SykepengeUtbetaling::fom)
                .min(Comparator.naturalOrder())
                .orElse(null);
        LocalDate max = grunnlag.utbetalingerNonNull().stream()
        ...
        var grunnlagBuilder = InfotrygdYtelseGrunnlag.getBuilder()
                ...
                .medVedtattTidspunkt(
                        grunnlag.vedtattTidspunkt() != null ? grunnlag.vedtattTidspunkt() : LocalDateTime.now())
                .medVedtaksPeriodeFom(brukPeriode.fom())
                .medVedtaksPeriodeTom(brukPeriode.tom());

...
public class FpsakKlient {
    ...
    public FpsakKlient() {
        ...
        this.endpointFpsak =
                UriBuilder.fromUri(restConfig.endpoint()).path(CALLBACK_PATH).build();
    }

    public void sendCallback(CallbackDto callbackDto) {
        try {
            LOG.info("Sender callback til fpsak.");
            restClient.sendReturnOptional(
                    RestRequest.newPOSTJson(callbackDto, endpointFpsak, restConfig), String.class);

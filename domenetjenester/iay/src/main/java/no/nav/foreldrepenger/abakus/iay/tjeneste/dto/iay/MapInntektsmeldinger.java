...
public class MapInntektsmeldinger {
    ...
    private static final Comparator<RefusjonDto> COMP_ENDRINGER_REFUSJON =
            Comparator.comparing((RefusjonDto re) -> re.getFom(), Comparator.nullsLast(Comparator.naturalOrder()));

    private static final Comparator<GraderingDto> COMP_GRADERING = Comparator.comparing(
                    (GraderingDto dto) -> dto.getPeriode().getFom(), Comparator.nullsFirst(Comparator.naturalOrder()))
    ...
    private static final Comparator<NaturalytelseDto> COMP_NATURALYTELSE = Comparator.comparing(
                    (NaturalytelseDto dto) -> dto.getPeriode().getFom(),
                    Comparator.nullsFirst(Comparator.naturalOrder()))
            .thenComparing(dto -> dto.getPeriode().getTom(), Comparator.nullsLast(Comparator.naturalOrder()))
            .thenComparing(
                    dto -> dto.getType() == null ? null : dto.getType().getKode(),
    ...
    private static final Comparator<UtsettelsePeriodeDto> COMP_UTSETTELSE = Comparator.comparing(
                    (UtsettelsePeriodeDto dto) -> dto.getPeriode().getFom(),
                    Comparator.nullsFirst(Comparator.naturalOrder()))
            .thenComparing(dto -> dto.getPeriode().getTom(), Comparator.nullsLast(Comparator.naturalOrder()))
            .thenComparing(
                    dto -> dto.getUtsettelseÅrsakDto() == null
                            ? null
                            : dto.getUtsettelseÅrsakDto().getKode(),
                    Comparator.nullsLast(Comparator.naturalOrder()));

    private static final Comparator<FraværDto> COMP_FRAVÆR = Comparator.comparing(
                    (FraværDto dto) -> dto.getPeriode().getFom(), Comparator.nullsFirst(Comparator.naturalOrder()))
            .thenComparing(dto -> dto.getPeriode().getTom(), Comparator.nullsLast(Comparator.naturalOrder()));
    ...
    private static final Comparator<InntektsmeldingDto> COMP_INNTEKTSMELDING = Comparator.comparing(
            ...
            .thenComparing(
                    im -> im.getArbeidsforholdRef() == null
                            ? null
                            : im.getArbeidsforholdRef().getAbakusReferanse(),
                    Comparator.nullsLast(Comparator.naturalOrder()));

    public static InntektsmeldingerDto mapUnikeInntektsmeldingerFraGrunnlag(
            Map<Inntektsmelding, ArbeidsforholdInformasjon> inntektsmeldingerMap) {
    ...
    public static RefusjonskravDatoerDto mapRefusjonskravdatoer(
            Set<Inntektsmelding> inntektsmeldinger, InntektArbeidYtelseGrunnlag nyesteGrunnlag) {
        if (nyesteGrunnlag.getInntektsmeldinger().isEmpty()) {
            return new RefusjonskravDatoerDto(Collections.emptyList());
        }
        List<RefusjonskravDatoDto> refusjonskravDatoList =
                lagRefusjonskravdatoPerArbeidsgiver(inntektsmeldinger, nyesteGrunnlag);

        return new RefusjonskravDatoerDto(refusjonskravDatoList);
    }

    private static List<RefusjonskravDatoDto> lagRefusjonskravdatoPerArbeidsgiver(
            Set<Inntektsmelding> inntektsmeldinger, InntektArbeidYtelseGrunnlag nyesteGrunnlag) {
        List<RefusjonskravDatoDto> refusjonskravDatoList = new ArrayList<>();
        Map<Arbeidsgiver, Optional<LocalDate>> førsteRefusjonsdatoMap = nyesteGrunnlag
                .getInntektsmeldinger()
                .map(MapInntektsmeldinger::lagFørsteRefusjonsdatoMap)
                .orElse(Map.of());
        førsteRefusjonsdatoMap.forEach((arbeidsgiver, førsteDatoMedRefusjon) ->
                finnFørsteDatoForInnsendelseAvRefusjonskrav(inntektsmeldinger, arbeidsgiver)
                        .ifPresent(innsendingDato -> refusjonskravDatoList.add(new RefusjonskravDatoDto(
                                mapTilAktør(arbeidsgiver),
                                innsendingDato.toLocalDate(),
                                førsteDatoMedRefusjon.orElse(null),
    ...
    private static Map<Arbeidsgiver, Optional<LocalDate>> lagFørsteRefusjonsdatoMap(
        ...
        return inntektsmeldingAggregat.getInntektsmeldinger().stream()
                ...
                .collect(Collectors.toMap(
                        ...
                        MapInntektsmeldinger::finnFørsteDatoMedRefusjon,
    ...
    private static Optional<LocalDateTime> finnFørsteDatoForInnsendelseAvRefusjonskrav(
            Set<Inntektsmelding> inntektsmeldinger, Arbeidsgiver arbeidsgiver) {
    ...
    private static Optional<LocalDate> finnFørsteDatoMedRefusjon(Inntektsmelding im) {
        return harRefusjonFraStart(im)
                ? Optional.ofNullable(utledStartDato(im))
                : im.getEndringerRefusjon().stream().map(Refusjon::getFom).min(Comparator.naturalOrder());
    }
    ...
    private static LocalDate utledStartDato(Inntektsmelding im) {
        if (im.getStartDatoPermisjon() != null) {
            ...
            return im.getOppgittFravær().stream()
    ...
    private static boolean harEndringerIRefusjon(Inntektsmelding im) {
        return im.getEndringerRefusjon().stream()
                .anyMatch(refusjon -> !refusjon.getRefusjonsbeløp().erNulltall());
    }

    private static boolean harRefusjonFraStart(Inntektsmelding im) {
        return im.getRefusjonBeløpPerMnd() != null
                && !im.getRefusjonBeløpPerMnd().erNulltall();
    }

    private static List<InntektsmeldingDto> mapUnikeInntektsmeldinger(
            Map<Inntektsmelding, ArbeidsforholdInformasjon> inntektsmeldingerMap) {
    ...
    public static class MapTilDto {
        ...
        public InntektsmeldingDto mapInntektsmelding(Inntektsmelding im) {
            ...
            var inntektsmeldingDto = new InntektsmeldingDto(
                            arbeidsgiver, journalpostId, innsendingstidspunkt, mottattDato)
                    ...
                    .medRefusjonsBeløpPerMnd(
                            im.getRefusjonBeløpPerMnd() == null
                                    ? null
                                    : im.getRefusjonBeløpPerMnd().getVerdi())
                    .medStartDatoPermisjon(im.getStartDatoPermisjon())
                    .medNærRelasjon(im.getErNærRelasjon());

            inntektsmeldingDto.medEndringerRefusjon(im.getEndringerRefusjon().stream()
                    .map(this::mapEndringRefusjon)
                    .sorted(COMP_ENDRINGER_REFUSJON)
                    .collect(Collectors.toList()));

            inntektsmeldingDto.medGraderinger(im.getGraderinger().stream()
                    .map(this::mapGradering)
                    .sorted(COMP_GRADERING)
                    .collect(Collectors.toList()));

            inntektsmeldingDto.medNaturalytelser(im.getNaturalYtelser().stream()
                    .map(this::mapNaturalytelse)
                    .sorted(COMP_NATURALYTELSE)
                    .collect(Collectors.toList()));

            inntektsmeldingDto.medUtsettelsePerioder(im.getUtsettelsePerioder().stream()
                    .map(this::mapUtsettelsePeriode)
                    .sorted(COMP_UTSETTELSE)
                    .collect(Collectors.toList()));

            inntektsmeldingDto.medOppgittFravær(im.getOppgittFravær().stream()
                    .map(this::mapOppgittFravær)
                    .sorted(COMP_FRAVÆR)
                    .collect(Collectors.toList()));
        ...
        private RefusjonDto mapEndringRefusjon(Refusjon refusjon) {
            return new RefusjonDto(
                    refusjon.getFom(), refusjon.getRefusjonsbeløp().getVerdi());
        }
        ...
        private GraderingDto mapGradering(Gradering gradering) {
            ...
            return new GraderingDto(
                    new Periode(periode.getFomDato(), periode.getTomDato()), arbeidstidProsent.getVerdi());
        }
        ...
        private ArbeidsforholdRefDto mapArbeidsforholdsId(
                @SuppressWarnings("unused") Arbeidsgiver arbeidsgiver,
                InternArbeidsforholdRef internRef,
                EksternArbeidsforholdRef eksternRef) {
            if ((internRef == null || internRef.getReferanse() == null)
                    && (eksternRef == null || eksternRef.getReferanse() == null)) {
                return null;
            } else if (internRef != null
                    && eksternRef != null
                    && internRef.getReferanse() != null
                    && eksternRef.getReferanse() != null) {
                return new ArbeidsforholdRefDto(
                        internRef.getReferanse(),
                        eksternRef.getReferanse(),
                        no.nav.abakus.iaygrunnlag.kodeverk.Fagsystem.AAREGISTERET);
            } else {
                throw new IllegalStateException(
                        "Både internArbeidsforholdRef og eksternArbeidsforholdRef må være satt (eller begge ikke satt), har nå internRef="
                                + internRef + ", eksternRef=" + eksternRef);
            }
        }
    }
    ...
    public static class MapFraDto {
        ...
        public InntektsmeldingAggregat map(
                ArbeidsforholdInformasjonBuilder arbeidsforholdInformasjon, InntektsmeldingerDto dto) {
            if (dto == null) {
                return null;
            }
        ...
        private Inntektsmelding mapInntektsmelding(
                ArbeidsforholdInformasjonBuilder arbeidsforholdInformasjon, InntektsmeldingDto dto) {
            var arbeidsforholdRef = dto.getArbeidsforholdRef();
            var internRef = InternArbeidsforholdRef.ref(
                    arbeidsforholdRef == null ? null : arbeidsforholdRef.getAbakusReferanse());
            var eksternRef = EksternArbeidsforholdRef.ref(
                    arbeidsforholdRef == null ? null : arbeidsforholdRef.getEksternReferanse());
            ...
            if (eksternRef.gjelderForSpesifiktArbeidsforhold() && !internRef.gjelderForSpesifiktArbeidsforhold()) {
                ...
                if (eksternRef.gjelderForSpesifiktArbeidsforhold()
                        && internRef.gjelderForSpesifiktArbeidsforhold()
                        && arbeidsforholdInformasjon.erUkjentReferanse(arbeidsgiver, internRef)) {
                    arbeidsforholdInformasjon.leggTilNyReferanse(
                            new ArbeidsforholdReferanse(arbeidsgiver, internRef, eksternRef));
                }
            }
            var journalpostId = dto.getJournalpostId().getId();
            var innsendingstidspunkt = dto.getInnsendingstidspunkt()
            ...
            dto.getNaturalytelser().stream()
                    .map(ny -> {
                        ...
                        return new NaturalYtelse(
                                periode.getFom(), periode.getTom(), ny.getBeløpPerMnd(), naturalYtelseType);
                    })
                    .forEach(builder::leggTil);

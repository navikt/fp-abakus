...
public class MapAktørInntekt {
    ...
    private static final Comparator<UtbetalingDto> COMP_UTBETALING = Comparator.comparing(
                    (UtbetalingDto dto) ->
                            dto.getKilde() == null ? null : dto.getKilde().getKode(),
                    Comparator.nullsLast(Comparator.naturalOrder()))
            .thenComparing(
                    dto -> dto.getUtbetaler() == null
                            ? null
                            : dto.getUtbetaler().getIdent(),
                    Comparator.nullsLast(Comparator.naturalOrder()));

    private static final Comparator<UtbetalingsPostDto> COMP_UTBETALINGSPOST = Comparator.comparing(
                    (UtbetalingsPostDto dto) -> dto.getInntektspostType().getKode(),
                    Comparator.nullsLast(Comparator.naturalOrder()))
            .thenComparing(
                    (UtbetalingsPostDto dto) -> dto.getInntektYtelseType() == null
                            ? null
                            : dto.getInntektYtelseType().getKode(),
                    Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    static class MapFraDto {
        ...
        List<AktørInntektBuilder> map(Collection<InntekterDto> dtos) {
            ...
            var builders = dtos.stream()
                    .map(idto -> {
                        var builder = aggregatBuilder.getAktørInntektBuilder(tilAktørId(idto.getPerson()));
                        idto.getUtbetalinger()
                                .forEach(utbetalingDto -> builder.leggTilInntekt(mapUtbetaling(utbetalingDto)));
                        return builder;
                    })
    ...
    static class MapTilDto {
        ...
        private InntekterDto mapTilInntekt(AktørInntekt ai) {
            InntekterDto dto =
                    new InntekterDto(new AktørIdPersonident(ai.getAktørId().getId()));
        ...
        private List<UtbetalingDto> tilUtbetalinger(Collection<Inntekt> inntekter) {
            return inntekter.stream()
                    .map(in -> tilUtbetaling(in))
                    .sorted(COMP_UTBETALING)
        ...
        private List<UtbetalingsPostDto> tilPoster(Collection<Inntektspost> inntektspost) {
            return inntektspost.stream()
                    .map(this::tilPost)
                    .sorted(COMP_UTBETALINGSPOST)
                    .collect(Collectors.toList());
        }

        private UtbetalingsPostDto tilPost(Inntektspost inntektspost) {
            var periode = new Periode(
                    inntektspost.getPeriode().getFomDato(),
                    inntektspost.getPeriode().getTomDato());

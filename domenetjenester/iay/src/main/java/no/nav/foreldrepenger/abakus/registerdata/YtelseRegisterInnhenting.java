...
public class YtelseRegisterInnhenting {
    ...
    void byggYtelser(Kobling behandling, AktørId aktørId, Interval periode,
        ...
        for (InfotrygdSakOgGrunnlag ytelse : sammenstilt) {
            YtelseType type = ytelse.getGrunnlag().map(YtelseBeregningsgrunnlag::getType)
                    .orElse(ytelse.getSak().getYtelseType());
        ...
        if (medGrunnlag) {
            var arena = innhentingSamletTjeneste.hentYtelserTjenester(aktørId, periode);
    ...
    private void innhentFraYtelsesRegister(AktørId aktørId, Kobling kobling,
            InntektArbeidYtelseAggregatBuilder.AktørYtelseBuilder builder) {
        DatoIntervallEntitet opplysningsperiode = kobling.getOpplysningsperiode();
        List<VedtattYtelse> vedtatteYtelser = vedtakYtelseRepository.hentYtelserForIPeriode(aktørId,
                opplysningsperiode.getFomDato(), opplysningsperiode.getTomDato());

        for (VedtattYtelse vedtattYtelse : vedtatteYtelser) {
            YtelseBuilder ytelseBuilder = builder.getYtelselseBuilderForType(vedtattYtelse.getKilde(),
                    vedtattYtelse.getYtelseType(), vedtattYtelse.getSaksnummer());
    ...
    private void mapAnvisninger(VedtattYtelse vedtattYtelse, YtelseBuilder ytelseBuilder) {
        vedtattYtelse.getYtelseAnvist().forEach(anvisning -> {
            ...
            DatoIntervallEntitet periode = DatoIntervallEntitet.fraOgMedTilOgMed(anvisning.getAnvistFom(),
                    anvisning.getAnvistTom());
            anvistBuilder.medAnvistPeriode(periode)
                    ...
                    .medUtbetalingsgradProsent(
                            anvisning.getUtbetalingsgradProsent().map(Stillingsprosent::getVerdi).orElse(null));
        });
    }

    /**
     * Bestemmer hvilke {@link YtelseType} som skal kopieres inn for søker og
     * annenpart.
     */
    private boolean skalKopiereTilYtelse(Kobling behandling, AktørId aktørId, YtelseType relatertYtelseType) {
        ...
        return !aktørId.equals(behandling.getAktørId())
                && List.of(YtelseType.FORELDREPENGER, YtelseType.ENGANGSSTØNAD).contains(relatertYtelseType);
    }

    private void oversettSakGrunnlagTilYtelse(InntektArbeidYtelseAggregatBuilder.AktørYtelseBuilder aktørYtelseBuilder,
            InfotrygdSakOgGrunnlag ytelse) {
        ryddBortFeilaktigeInnhentedeYtelser(aktørYtelseBuilder);
        YtelseBuilder ytelseBuilder = aktørYtelseBuilder
                .getYtelselseBuilderForType(Fagsystem.INFOTRYGD, ytelse.getSak().getYtelseType(),
        ...
        ytelse.getGrunnlag().ifPresent(grunnlag -> {
            for (YtelseBeregningsgrunnlagVedtak vedtak : grunnlag.getVedtak()) {
                final DatoIntervallEntitet intervall = vedtak.getTom() == null
                        ? DatoIntervallEntitet.fraOgMed(vedtak.getFom())
                ...
                ytelseBuilder.medYtelseAnvist(ytelseBuilder.getAnvistBuilder()
                        ...
                        .medUtbetalingsgradProsent(
                                vedtak.getUtbetalingsgrad() != null ? new BigDecimal(vedtak.getUtbetalingsgrad())
                                        : null)
                        .build());
            }
    ...
    private void ryddBortFeilaktigeInnhentedeYtelser(
            InntektArbeidYtelseAggregatBuilder.AktørYtelseBuilder aktørYtelseBuilder) {
        aktørYtelseBuilder.tilbakestillYtelserFraKildeMedFeil(Fagsystem.INFOTRYGD, YtelseType.PÅRØRENDESYKDOM,
                TemaUnderkategori.UDEFINERT);
    }

    private YtelseGrunnlag oversettYtelseGrunnlag(YtelseBeregningsgrunnlag grunnlag,
            YtelseGrunnlagBuilder grunnlagBuilder) {
    ...
    private void oversettMeldekortUtbetalingsgrunnlagTilYtelse(
        ...
        YtelseBuilder ytelseBuilder = aktørYtelseBuilder.getYtelselseBuilderForType(ytelse.getKilde(),
                ytelse.getYtelseType(), ytelse.getSaksnummer(), periode, førsteMeldekortFom);
        ytelseBuilder
                .medPeriode(periode)
        ...
        for (MeldekortUtbetalingsgrunnlagMeldekort meldekort : ytelse.getMeldekortene()) {
            ytelseBuilder.medYtelseAnvist(ytelseBuilder.getAnvistBuilder()
                    .medAnvistPeriode(DatoIntervallEntitet.fraOgMedTilOgMed(meldekort.getMeldekortFom(),
                            meldekort.getMeldekortTom()))
    ...
    private DatoIntervallEntitet utledMeldekortVedtaksPeriode(MeldekortUtbetalingsgrunnlagSak sak,
            Optional<LocalDate> førsteMeldekortFom) {
    ...
    private LocalDate utledFomFraSakEllerMeldekortene(MeldekortUtbetalingsgrunnlagSak sak,
        ...
        return førsteMeldekortFom
                .orElseGet(() -> sak.getVedtattDato() != null ? sak.getVedtattDato() : sak.getKravMottattDato());
    }
    ...
    private void leggTilArbeidsforhold(YtelseGrunnlagBuilder ygBuilder,
            List<YtelseBeregningsgrunnlagArbeidsforhold> arbeidsforhold) {

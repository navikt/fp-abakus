...
public class LåsRepository {
    ...
    private Long lås(final Long behandlingId, LockModeType lockModeType) {
        Object[] result = (Object[]) entityManager
                .createQuery("select k.id, k.versjon from Kobling k where k.id=:id and k.aktiv=true")
    ...
    private Object verifisertLås(Long id) {
        ...
        if (entity == null) {
            throw new TekniskException(
                    "FP-131239",
                    String.format("Fant ikke entitet for låsing [%s], id=%s.", Kobling.class.getSimpleName(), id));
        } else {

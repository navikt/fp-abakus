...
class LønnskompensasjonRepositoryTest {
    ...
    void skal_håndtere_lagring_rett() {
        var anvist = LønnskompensasjonAnvist.LønnskompensasjonAnvistBuilder.ny()
                .medAnvistPeriode(
                        IntervallEntitet.fraOgMedTilOgMed(LocalDate.of(2020, 4, 20), LocalDate.of(2020, 4, 20)))
        ...
        vedtak.setPeriode(IntervallEntitet.fraOgMedTilOgMed(
        ...
        final var vedtakFraRepo = repository.hentLønnskompensasjonForIPeriode(
                AKTØR_ID, LocalDate.now().minusMonths(17), LocalDate.now());
        ...
        final var oppdatertVedtattVedtak =
                repository
    ...
    void skal_forkaste_vedtak_som_likt() {
        ...
        final var vedtakFraRepo = repository.hentLønnskompensasjonForIPeriode(
                AKTØR_ID, LocalDate.now().minusYears(1), LocalDate.now());
        assertThat(vedtakFraRepo).hasSize(1);

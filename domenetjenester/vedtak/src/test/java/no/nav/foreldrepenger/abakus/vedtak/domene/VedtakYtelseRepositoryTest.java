...
class VedtakYtelseRepositoryTest {
    ...
    void skal_håndtere_lagring_rett() {
        ...
        final var builder =
                repository.opprettBuilderFor(aktørId, saksnummer, Fagsystem.FPSAK, YtelseType.FORELDREPENGER);
        ...
        final var vedtattYtelser =
                repository.hentYtelserForIPeriode(aktørId, LocalDate.now().minusDays(30), LocalDate.now());
        assertThat(vedtattYtelser).hasSize(1);

        final var nyBuilder =
                repository.opprettBuilderFor(aktørId, saksnummer, Fagsystem.FPSAK, YtelseType.FORELDREPENGER);
        final var intervallEntitet = IntervallEntitet.fraOgMedTilOgMed(
        ...
        final var oppdatertVedtattYtelser =
                repository.hentYtelserForIPeriode(aktørId, LocalDate.now().minusDays(30), LocalDate.now());

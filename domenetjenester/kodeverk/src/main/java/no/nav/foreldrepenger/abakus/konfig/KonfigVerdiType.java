package no.nav.foreldrepenger.abakus.konfig;
...
public class KonfigVerdiType extends Kodeliste {
    ...
    public static final KonfigVerdiType DATE_TYPE = internOpprett("DATE", KonfigVerdiType::validerLocalDate); //$NON-NLS-1$
    ...
    static boolean validerPeriod(String str) {
        Period.parse(str);  // NOSONAR
        return true;
    }

    static boolean validerDuration(String str) {
        Duration.parse(str); // NOSONAR
        return true;
    }

    private static boolean validerLocalDate(String s) {
        LocalDate.parse(s);// NOSONAR
        return true;
    }

...
public class ArbeidsforholdOverstyringerDataDump implements DebugDump {
    ...
    public List<DumpOutput> dump(DumpKontekst dumpKontekst) {
        var sql = "select" + " k.saksnummer, " + " k.id as kobling_id, "
                ...
                + " cast(k.annen_part_aktoer_id as varchar) annen_part_aktoer_id, " + " k.aktiv as k_aktiv, "
                + " gr.id as grunnlag_id, "
                + " gr.aktiv as gr_aktiv," + " gr.informasjon_id as gr_informasjon_id,"
                + " cast(gr.grunnlag_referanse as varchar) grunnlag_referanse,"
                + " cast(ov.arbeidsforhold_intern_id_ny as varchar) as ov_arbeidsforhold_intern_id_ny,"
                + " ov.arbeidsgiver_aktor_id as ov_arbeidsgiver_aktor_id,"
                + " ov.arbeidsgiver_orgnr as ov_arbeidsgiver_orgnr,"
                + " ov.begrunnelse as ov_begrunnelse," + " ov.handling_type as ov_handling_type,"
                + " ov.bekreftet_tom_dato as ov_bekreftet_tom_dato,"
                + " ov.arbeidsgiver_navn as ov_arbeidsgiver_navn,"
                + " ov.bekreftet_permisjon_status as ov_bekreftet_permisjon_status,"
                + " ov.bekreftet_permisjon_fom as ov_bekreftet_permisjon_fom,"
                + " ov.bekreftet_permisjon_tom as ov_bekreftet_permisjon_tom,"
                + " ov.stillingsprosent as ov_stillingsprosent,"
                + " replace(cast(ov.opprettet_tid as varchar), ' ', 'T') ov_opprettet_tid"

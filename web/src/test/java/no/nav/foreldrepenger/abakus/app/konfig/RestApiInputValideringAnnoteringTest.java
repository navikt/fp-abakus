...
class RestApiInputValideringAnnoteringTest extends RestApiTester {
    ...
    private Function<Method, String> printKlasseOgMetodeNavn =
            (method -> String.format("%s.%s", method.getDeclaringClass(), method.getName()));
    ...
    @Test
    void
            alle_felter_i_objekter_som_brukes_som_inputDTO_skal_enten_ha_valideringsannotering_eller_være_av_godkjent_type()
                    throws Exception {
        for (Method method : finnAlleRestMetoder()) {
            for (int i = 0; i < method.getParameterCount(); i++) {
                ...
                assertThat(method.getParameterTypes()[i].isAssignableFrom(String.class)
                                && !parameter.isAnnotationPresent(jakarta.validation.constraints.Pattern.class))
                        .as(
